...
import pytest
from lnp_formulator.models import FrontierPayload, ExecutionStatus
from lnp_formulator.engine import FrontierDomainEngine
from lnp_formulator.agents import NPRatioCalculatorAgent, ApparentPKaPredictorAgent, EncapsulationEfficiencyAgent, LNPFormulatorCoordinator
from lnp_formulator.cli import main


def test_sub_agents():
    a1 = NPRatioCalculatorAgent()
    ...
    a3 = EncapsulationEfficiencyAgent()
...
def test_coordinator():
    coord = LNPFormulatorCoordinator()
    ...
    assert "Lipid Nanoparticle (LNP) Quality Metrics" in ans or "specifications" in ans
